...
public class KDTreeUtil {
    ...
    private void buildDetail(Node node, ArrayList<ArrayList<Double>> data, int dimensions) {
        ...
        node.partitionValue = UtilZ.median(data, node.partition_dimension);
        ...
        for (ArrayList<Double> d : data) {
            if (d.get(node.partition_dimension) < node.partitionValue) {
                left.add(d);
            } else if (d.get(node.partition_dimension) > node.partitionValue) {
                right.add(d);
            }
        }
        for (ArrayList<Double> d : data) {
    ...
    public ArrayList<Double> query(ArrayList<Double> input, double[] std) {
        ...
        while (!node.isLeaf) {
            if (input.get(node.partition_dimension) < node.partitionValue) {
    ...
    private static class UtilZ {
        ...
        static double distance(ArrayList<Double> a, ArrayList<Double> b, double[] std) {
            ...
            for (int i = 0; i < a.size(); i++) {
                if (a.get(i) != null && b.get(i) != null) sum += Math.pow((a.get(i) - b.get(i)) / std[i], 2);
            }
            sum = sqrt(sum);
            return sum;
        }
